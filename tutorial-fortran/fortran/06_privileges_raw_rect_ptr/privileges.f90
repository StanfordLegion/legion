...
function top_level_task(tdata, tdatalen, userdata, userlen, p)
   ...
   ! common HELLO_WORLD_TASK_ID
    ...
    init_launcher_x = legion_task_launcher_create_f(INIT_TASK_ID, task_args, pred, 0, tag)
    rr_ix = legion_task_launcher_add_region_requirement_logical_region_f(init_launcher_x, input_lr, & 
                                                                       WRITE_DISCARD, EXCLUSIVE, &
                                                                       input_lr, tag, verified)
    call legion_task_launcher_add_field_f(init_launcher_x, rr_ix, 0, inst)
    init_task_future = legion_task_launcher_execute_f(runtime, ctx, init_launcher_x)
    
    !init task for Y
    i = 1
    task_args%args = c_loc(i)
    task_args%arglen = c_sizeof(i)
    init_launcher_y = legion_task_launcher_create_f(INIT_TASK_ID, task_args, pred, 0, tag)
    rr_iy = legion_task_launcher_add_region_requirement_logical_region_f(init_launcher_y, input_lr, & 
                                                                       WRITE_DISCARD, EXCLUSIVE, &
                                                                       input_lr, tag, verified)
    call legion_task_launcher_add_field_f(init_launcher_y, rr_iy, 1, inst)
    init_task_future = legion_task_launcher_execute_f(runtime, ctx, init_launcher_y)
    ...
    task_args%args = c_loc(i)
    task_args%arglen = c_sizeof(i)
    daxpy_launcher = legion_task_launcher_create_f(DAXPY_TASK_ID, task_args, pred, 0, tag)
    rr_cxy = legion_task_launcher_add_region_requirement_logical_region_f(daxpy_launcher, input_lr, & 
                                                                       READ_ONLY, EXCLUSIVE, &
                                                                       input_lr, tag, verified)
    ...
    rr_cz = legion_task_launcher_add_region_requirement_logical_region_f(daxpy_launcher, output_lr, & 
                                                                       WRITE_DISCARD, EXCLUSIVE, &
                                                                       output_lr, tag, verified)
    call legion_task_launcher_add_field_f(daxpy_launcher, rr_cz, 2, inst)
    daxpy_task_future = legion_task_launcher_execute_f(runtime, ctx, daxpy_launcher)
