...
contains
  subroutine init_task(tdata, tdatalen, userdata, userlen, p)
    ...
    call legion_accessor_array_1d_raw_rect_ptr_f(accessor, index_rect, subrect, offset, raw_ptr_x)
    
    Print *, "Init Task!", rrfid, fid, index_rect%lo%x(0), arglen, raw_ptr_x
 ...
 !   Print *, "Daxpy Task!", task_arg, arglen
  ...
  subroutine check_task(tdata, tdatalen, userdata, userlen, p)
    ...
    Print *, "Check Task!", task_arg, arglen, raw_ptr_x, raw_ptr_y, raw_ptr_z
    do i = index_rect%lo%x(0), index_rect%hi%x(0)
      point%x(0) = i
  ...
  subroutine top_level_task(tdata, tdatalen, userdata, userlen, p)
   ...
   ! common HELLO_WORLD_TASK_ID
    ...
    call legion_index_launcher_create_f(INIT_TASK_ID, color_domain, task_args, arg_map, pred, .false., 0, tag, init_launcher_x)
    ...
    task_args%args = c_loc(i)
    task_args%arglen = c_sizeof(i)
    call legion_index_launcher_create_f(INIT_TASK_ID, color_domain, task_args, arg_map, pred, .false., 0, tag, init_launcher_y)
    ...
    task_args%args = c_loc(i)
    task_args%arglen = c_sizeof(i)
    call legion_index_launcher_create_f(DAXPY_TASK_ID, color_domain, task_args, arg_map, pred, .false., 0, tag, daxpy_launcher)
...
end module daxpy_partition

Program daxpy_partition_accessor
  use legion_fortran
  use iso_c_binding
  use daxpy_partition
  implicit none
...
End Program daxpy_partition_accessor
