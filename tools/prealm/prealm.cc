...
namespace PRealm {
  ...
  {
    for (std::vector<RegionInstance>::const_iterator it =
  ...
  void ThreadProfiler::record_barrier_use(Event bar)
  {
    ...
    // they have a reduction operator and if they don't then substitute the arrival
    // timing reduction operator similar to what Legion does, although then if we
    // have mixed barriers some with reduction and some without we'll need some way
  ...
  void ThreadProfiler::record_reservation_acquire(Reservation r, Event result, Event precondition)
  {
    Profiler &profiler = Profiler::get_profiler();
    if (profiler.no_critical_paths)
      return;
    ReservationAcquireInfo &info = reservation_acquire_infos.emplace_back(ReservationAcquireInfo());
    info.performed = Realm::Clock::current_time_in_nanoseconds();
    info.result = result;
    info.precondition = precondition;
    if (precondition.is_barrier())
  ...
  void ThreadProfiler::process_response(ProfilingResponse &response)
  {
    ...
    switch (args->kind)
    {
      case FILL_PROF:
      {
        ...
        profiler.update_footprint(sizeof(FillInfo) +
      ...
      case COPY_PROF:
      {
        ...
        profiler.update_footprint(sizeof(CopyInfo) +
      ...
      case TASK_PROF:
      {
        ...
        if (response.get_measurement(timeline_gpu)) {
          ...
          profiler.update_footprint(sizeof(info) + 
              num_intervals * sizeof(WaitInfo), this);
        } else {
  ...
  void Profiler::serialize(const ThreadProfiler::ProcDesc &proc_desc) const
  {
    ...
    for (size_t i=0; i<Realm::Cuda::UUID_SIZE; i++) {
      pr_fwrite(f, (char*)&(proc_desc.cuda_device_uuid[i]),
          sizeof(char));
    }
#else
  ...
  void Profiler::serialize(const ThreadProfiler::TaskInfo &task_info) const
  {
    ...
    for (std::vector<ThreadProfiler::WaitInfo>::const_iterator it =
          task_info.wait_intervals.begin(); it != task_info.wait_intervals.end(); it++) {
  ...
  void Profiler::serialize(const ThreadProfiler::InstTimelineInfo &inst_timeline_info) const
  {
  ...
  void Profiler::serialize(const ThreadProfiler::ProfTaskInfo &proftask_info) const
  {
    ...
    pr_fwrite(f, (char*)&ID, sizeof(ID));
    pr_fwrite(f, (char*)&(proftask_info.proc_id), 
    ...
    pr_fwrite(f, (char*)&(proftask_info.creator),
  ...
  void Profiler::record_affinities(std::vector<Memory> &memories_to_log)
  {
    while (!memories_to_log.empty())
    {
            ...
            memory_affinities.begin(); mit != memory_affinities.end(); mit++)
      {
        ...
        const ThreadProfiler::ProcMemDesc info =
  ...
  bool Runtime::init(int *argc, char ***argv)
  {
    ...
    if(!Realm::Runtime::create_configs(*argc, *argv)) return false;
  ...
  Event Processor::register_task(TaskFuncID task_id,
                        ...
                        const void *user_data, size_t user_data_len) const
  {
    Profiler &profiler = Profiler::get_profiler();
    profiler.record_task(task_id);
    profiler.record_variant(task_id, kind());
    return Realm::Processor::register_task(task_id, codedesc, prs, user_data, user_data_len);
  }

  /*static*/ Event Processor::register_task_by_kind(Kind kind, bool global, TaskFuncID task_id,
                        const CodeDescriptor& codedesc,
                        const ProfilingRequestSet& prs,
  ...
  bool Runtime::register_task(Processor::TaskFuncID task_id, Processor::TaskFuncPtr taskptr)
  {
    ...
    profiler.record_task(task_id);
    // Get all the local processors and record that we have a variant for each kind of local processor
    Realm::Machine::ProcessorQuery local_procs(Machine::get_machine());
    local_procs.local_address_space();
    std::vector<Processor::Kind> kinds;
    for (Realm::Machine::ProcessorQuery::iterator it = local_procs.begin();
