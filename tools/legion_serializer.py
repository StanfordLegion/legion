...
class LegionProfASCIIDeserializer(LegionDeserializer):
    ...
    patterns = {
        ...
        "CopyInstInfo": re.compile(prefix + r'Prof Copy Inst Info (?P<src_inst>[a-f0-9]+) (?P<dst_inst>[a-f0-9]+) (?P<fevent>[a-f0-9]+) (?P<num_fields>[0-9]+) (?P<request_type>[0-9]+) (?P<num_hops>[0-9]+)'),
        "FillInfo": re.compile(prefix + r'Prof Fill Info (?P<op_id>[0-9]+) (?P<dst>[a-f0-9]+) (?P<create>[0-9]+) (?P<ready>[0-9]+) (?P<start>[0-9]+) (?P<stop>[0-9]+) (?P<fevent>[a-f0-9]+) (?P<num_requests>[0-9]+)'),
        "FillInstInfo": re.compile(prefix + r'Prof Fill Inst Info (?P<dst_inst>[a-f0-9]+) (?P<fevent>[a-f0-9]+) (?P<num_fields>[0-9]+)'),
