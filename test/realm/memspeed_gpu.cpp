...
double gpu_seqwr_test(void *buffer, size_t reps, size_t elements)
{
  ...
  hipEventRecord(t_start, hipGetTaskStream());
  hipLaunchKernelGGL(gpu_seqwr_kernel, dim3(grid_size), dim3(block_size ), 0, hipGetTaskStream(), (int *)buffer, reps, elements);
  hipEventRecord(t_end, hipGetTaskStream());
...
double gpu_seqrd_test(void *buffer, size_t reps, size_t elements)
{
  ...
  hipEventCreate(&t_end);
  hipEventRecord(t_start, hipGetTaskStream());
  hipLaunchKernelGGL(gpu_seqrd_kernel, dim3(grid_size), dim3(block_size ), 0, hipGetTaskStream(), (int *)buffer, reps, elements);
  hipEventRecord(t_end, hipGetTaskStream());
...
double gpu_rndwr_test(void *buffer, size_t reps, size_t elements)
{
  ...
  hipEventCreate(&t_end);
  hipEventRecord(t_start, hipGetTaskStream());
  hipLaunchKernelGGL(gpu_rndwr_kernel, dim3(grid_size), dim3(block_size ), 0, hipGetTaskStream(), (int *)buffer, reps, steps, elements);
  hipEventRecord(t_end, hipGetTaskStream());
...
double gpu_rndrd_test(void *buffer, size_t reps, size_t elements)
{
  ...
  hipEventCreate(&t_end);
  hipEventRecord(t_start, hipGetTaskStream());
  hipLaunchKernelGGL(gpu_rndrd_kernel, dim3(grid_size), dim3(block_size ), 0, hipGetTaskStream(), (int *)buffer, reps, steps, elements);
  hipEventRecord(t_end, hipGetTaskStream());
...
double gpu_latency_test(void *buffer, size_t reps, size_t elements)
{
  ...
  if(delta == 0) delta = 1;
  hipLaunchKernelGGL(gpu_latency_setup_kernel, dim3(grid_size), dim3(block_size ), 0, hipGetTaskStream(), (int *)buffer, delta, elements);

  hipEvent_t t_start, t_end;
  hipEventCreate(&t_start);
  hipEventCreate(&t_end);
  hipEventRecord(t_start, hipGetTaskStream());
  hipLaunchKernelGGL(gpu_latency_kernel, dim3(1), dim3(1 ), 0, hipGetTaskStream(), (int *)buffer, reps, steps, elements);
  hipEventRecord(t_end, hipGetTaskStream());
