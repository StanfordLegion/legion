...
void do_single_dim(Memory src_mem, Memory dst_mem, int log2_size,
  ...
  if (src_bounds.empty()) {
    ...
    for (int i = 0; i < N; i++) {
      ...
      src_bounds.back().hi[i] =
          domain.back().hi[i] +
          (((narrow_size > 0) && (narrow_size < TestConfig::pad_width))
               ? narrow_size
  ...
  InstanceLayoutConstraints ilc(field_sizes, /*block_size=*/0);
  ...
  for(unsigned i = 0; i < perms.size(); i++) {
    ...
    for (unsigned j = 0; j < perms.size(); j++) {
      ...
      dump_and_verify<N, T, FT>(src_inst, 0, src_is,
    ...
    for (std::vector<Memory>::const_iterator dst_it = dst_mems.begin();
      ...
      if (TestConfig::dim_mask == 1) {
        ...
        if (!TestConfig::do_unit_test) {
          ...
          /*do_single_dim_field_size<3, int>(
              *src_it, *dst_it, log2_buffer_size, TestConfig::narrow_dim, p, {"XYZ"}, {"XZY"},
              domain, src_bounds, dst_bounds, TestConfig::field_size);*/
