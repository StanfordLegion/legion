                    ...
                    const void *userdata, size_t userlen, Processor p)
{
  ...
  for (DefaultMap<Point2,RegionInstance>::const_iterator it =
        xp_insts.begin(); it != xp_insts.end(); it++)
    if (it->second.exists())
      it->second.destroy();
  for (DefaultMap<Point2,RegionInstance>::const_iterator it =
        xm_insts.begin(); it != xm_insts.end(); it++)
    if (it->second.exists())
      it->second.destroy();
  for (DefaultMap<Point2,RegionInstance>::const_iterator it =
        yp_insts.begin(); it != yp_insts.end(); it++)
    if (it->second.exists())
      it->second.destroy();
  for (DefaultMap<Point2,RegionInstance>::const_iterator it =
        ym_insts.begin(); it != ym_insts.end(); it++)
    if (it->second.exists())
      it->second.destroy();
}
