...
//  std::cout << "attaching file " << piece.shard_name << std::endl;
...
//  if(x_min != 16 || y_min != 16) {
//    return; 
//  }
  Rect<2> rect = dom.get_rect<2>();

  
  /* don't really need the physical instance passed to the task  - just the permissions*/ 
...
void PersistentRegion::write_persistent_subregions(Context ctx, LogicalRegion src_lr, LogicalPartition src_lp){
  ...
  IndexLauncher write_launcher(WRITE_VALUES_TASK_ID, this->dom,
...
//        write_launcher.add_wait_barrier(pieces->barrier);
  }
...
//        pieces[i].barrier = runtime->create_phase_barrier(ctx, 1);
        
        //        pieces[i].field_map = field_map;
        //IndexSpace is = pieces[i].lr.get_index_space();
        ...
        for(it_type iterator = field_map.begin(); iterator != field_map.end(); iterator++) {
            ...
            status = H5Lcreate_external(pieces[i].shard_name, iterator->second,
                                        link_group_2_id, std::to_string(i).c_str(),
                                        H5P_DEFAULT, H5P_DEFAULT);
            status = H5Acreate_by_name(link_group_2_id, std::to_string(i).c_str(),
                                       "dims", H5T_NATIVE_INT, attr_ds_id,
                                       H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
            hid_t link_ds_id = H5Dopen2(link_group_2_id, std::to_string(i).c_str(), H5P_DEFAULT); 
            hid_t attr_id = H5Aopen_name(link_ds_id, "dims");
        ...
        /* file create, let the writerxo go.. */
//        pieces[i].barrier = runtime->advance_phase_barrier(ctx, pieces[i].barrier);
        i++;
    }
//    H5Gclose(link_group_id);  should already be closed. 
