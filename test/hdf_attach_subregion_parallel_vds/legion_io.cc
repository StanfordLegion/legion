...
void PersistentRegion::create_persistent_subregions(Context ctx, const char *name,
  ...
  hid_t dset;
  hid_t dcpl;
  double fill_value = -1.;  // Fill value for VDS
  ...
  vdsdims[0] = 32;
  vdsdims[1] = 32;
  space = H5Screate_simple (2, vdsdims, NULL);
  ...
  //
...
#endif
        count[0] = x_max-x_min+1;
        count[1] = y_max-y_min+1;
        dataspace_id = H5Screate_simple(2, count, NULL);

	start[0] = pieces[i].dp[0]*count[0];
	start[1] = pieces[i].dp[1]*count[1];
      }
        break;
      case 3: {
	...
	src_space = H5Screate_simple (dim, count, NULL);
	status = H5Pset_virtual (dcpl, space, pieces[i].shard_name, "/bam/baz", src_space);
    ...
    dset = H5Dcreate2 (link_file_id, "/bam/baz/vds", H5T_NATIVE_DOUBLE, space, H5P_DEFAULT,
		       dcpl, H5P_DEFAULT);
