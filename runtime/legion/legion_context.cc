  ...
  namespace Internal {
    ...
    TaskContext::TaskContext(Runtime *rt, SingleTask *owner, int d,
      ...
      : runtime(rt), owner_task(owner), regions(reqs), depth(d),
        executing_processor(Processor::NO_PROC), overhead_profiler(NULL),
        implicit_profiler(NULL), task_executed(false), mutable_priority(false),
...
#undef MEM_NAMES
    ...
    InnerContext::InnerContext(Runtime *rt, SingleTask *owner,int d,bool finner,
      ...
      : TaskContext(rt, owner, d, reqs, inline_task, implicit_task),
        ...
        total_close_count(0), total_summary_count(0), total_tunable_count(0),
    ...
    //--------------------------------------------------------------------------
    void InnerContext::clone_requirement(unsigned idx,RegionRequirement &target)
    //--------------------------------------------------------------------------
    {
      if (idx >= regions.size())
      {
...
#undef DIMFUNC
    ...
    //--------------------------------------------------------------------------
    FieldSpace InnerContext::create_field_space(const char *provenance)
    //--------------------------------------------------------------------------
    {
      AutoRuntimeCall call(this);
    ...
    //--------------------------------------------------------------------------
    void InnerContext::add_created_region(LogicalRegion handle, bool task_local)
    //--------------------------------------------------------------------------
    {
    ...
    //--------------------------------------------------------------------------
    void InnerContext::register_region_creation(LogicalRegion handle,
                                               bool task_local)
    //--------------------------------------------------------------------------
    {
    ...
    //--------------------------------------------------------------------------
    void InnerContext::remove_deleted_requirements(
    ...
    //--------------------------------------------------------------------------
    void InnerContext::remove_deleted_fields(const std::set<FieldID> &to_free,
                                           const std::vector<unsigned> &indexes)
    //--------------------------------------------------------------------------
    {
      AutoLock priv_lock(privilege_lock);
    ...
    //--------------------------------------------------------------------------
    {
      ...
      else if (!mapping)
      {
              ...
              reorder_buffer.crbegin(); it != reorder_buffer.crend(); it++)
        {
          ...
          if (it->stage == COMMITTED_STAGE)
            continue;
          previous_events.insert(it->operation->get_program_order_event());
        }
        ...
        if ((executing_children_count == 0) && (executed_children_count == 0))
        {
          if (!children_complete_invoked)
          {
            ...
            child_completion_events.reserve(
                child_completion_events.size() + reorder_buffer.size()); 
            for (std::deque<ReorderBufferEntry>::const_iterator it =
                  reorder_buffer.begin(); it != reorder_buffer.end(); it++)
            {
#ifdef DEBUG_LEGION
