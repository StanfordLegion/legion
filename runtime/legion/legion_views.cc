...
namespace Legion {
  namespace Internal {
    ...
    //--------------------------------------------------------------------------
    {
      ...
      if (add_ref)
        add_collectable_reference();
      if (!to_collect.empty())
        collect_users(to_collect); 
      if (remove_ref && remove_collectable_reference())
        delete this;
    }
    ...
    //--------------------------------------------------------------------------
    {
      ...
      if (inst_view->is_logical_owner())
        inst_view->add_base_gc_ref(PENDING_GC_REF);
      else
    ...
    //--------------------------------------------------------------------------
    {
      if (inst_view->is_logical_owner())
      {
        if (inst_view->remove_base_gc_ref(PENDING_GC_REF))
          delete inst_view;
      }
      else
      {
    ...
    FillView::FillView(RegionTreeForest *ctx, DistributedID did,
      ...
      : DeferredView(ctx, encode_fill_did(did), register_now, map), 
#ifdef LEGION_SPY
        fill_op_uid(op_uid),
#endif
      ...
      if (value_size.load() == 0)
      {
        ...
        if (value_size.load() == 0)
        {
          ...
          DeferIssueFill args(this, op, fill_expr, trace_info, dst_fields,
                              manager, precondition, pred_guard);
    ...
    //--------------------------------------------------------------------------
    {
      ...
      else
      {
        ...
        if (issue_collect)
          defer_collect_user(get_manager(), term_event, collect_event);
        if (!wait_on_events.empty())
          return Runtime::merge_events(&trace_info, wait_on_events);
        else
          return ApEvent::NO_AP_EVENT;
    ...
    //--------------------------------------------------------------------------
    {
      ...
      // that we have all the valid gc event users
      if (is_logical_owner())
        add_base_gc_ref(PENDING_GC_REF);
      else
    ...
    {
      if (is_logical_owner())
        return remove_base_gc_ref(PENDING_GC_REF);
      else
    ...
    //--------------------------------------------------------------------------
    {
      ...
      const ApEvent copy_post = copy_expression->issue_copy(
          ...
          local_manager->get_unique_event());
      if (local_info.recording)
      {
        const UniqueInst dst_inst(local_view);
      ...
      if (!children.empty() || (local_views.size() > 1))
      {
        ...
        for (unsigned idx = 1; idx < local_views.size(); idx++)
        {
          ...
          const ApEvent copy_post = copy_expression->issue_copy(
              ...
              local_manager->tree_id, dst_manager->tree_id,
#endif
              copy_pre, predicate_guard, local_manager->get_unique_event(),
              dst_manager->get_unique_event());
          if (copy_post.exists())
          {
      ...
      if (local_views.size() > 1)
      {
        ...
        for (unsigned idx = 1; idx < local_views.size(); idx++)
        {
          ...
          const ApEvent reduced = copy_expression->issue_copy(op, inst_info,
              ...
              local_views[idx]->manager->get_unique_event(),
              local_views.front()->manager->get_unique_event());
...
#endif
              copy_pre, predicate_guard,
              local_view->manager->get_unique_event(),
              dst_view->manager->get_unique_event());
          // No need to record the output with the view
          // We'll do that at the end of the full all-reduce
          if (copy_post.exists())
          {
