...
namespace Legion {
  namespace Internal {
    ...
    //--------------------------------------------------------------------------
    {
      ...
      CopyUpdate *update = 
        new CopyUpdate(src_view, src_mask, expr, redop, helper);
      if (helper == NULL)
      ...
      if (!deferred.empty())
      {
        for (unsigned idx = 0; idx < deferred.size(); idx++)
        {
                ...
                src_expressions.begin(); it != src_expressions.end(); it++)
          {
            ...
            {
              if (overlap_size == it->first.second->get_volume())
                def->flatten(*this, dst_view, it->second,it->first.second,NULL);
              else
                def->flatten(*this, dst_view, it->second, overlap, NULL);
              // Compute the difference
              IndexSpaceExpression *diff_expr = 
                forest->subtract_index_spaces(it->first.first, overlap);
              remainders.insert(diff_expr, it->second);
            ...
            {
              def->flatten(*this, dst_view, it->second, it->first.first, NULL);
              if (remainders.empty())
                return;
              need_tighten = true;
            }
    ...
    PhysicalAnalysis::PhysicalAnalysis(Runtime *rt, Operation *o, unsigned idx, 
      ...
      : previous(rt->address_space), original_source(rt->address_space),
        runtime(rt), analysis_expr(e), op(o), index(idx), owns_op(false), 
    ...
    PhysicalAnalysis::PhysicalAnalysis(Runtime *rt, AddressSpaceID source, 
      ...
      : previous(prev), original_source(source), runtime(rt), analysis_expr(e),
        op(o), index(idx), owns_op(true), on_heap(h), remote_instances(NULL), 
    ...
    PhysicalAnalysis::PhysicalAnalysis(const PhysicalAnalysis &rhs)
      : previous(0), original_source(0), runtime(NULL), analysis_expr(NULL),
        op(NULL), index(0), owns_op(false), on_heap(false)
    //--------------------------------------------------------------------------
    {
      // should never be called
      assert(false);
    ...
    CopyAcrossAnalysis::CopyAcrossAnalysis(Runtime *rt, Operation *o, 
      ...
      : PhysicalAnalysis(rt, o, dst_idx, NULL/*expr*/, true/*on heap*/), 
    ...
    CopyAcrossAnalysis::CopyAcrossAnalysis(Runtime *rt, AddressSpaceID src, 
        ...
        const std::vector<unsigned> &dst_idxes, 
        const PhysicalTraceInfo &t_info, const bool perf)
      : PhysicalAnalysis(rt, src, prev, o,dst_idx,NULL/*expr*/,true/*on heap*/),
        src_mask(perf ? FieldMask() : initialize_mask(src_idxes)), 
        dst_mask(perf ? FieldMask() : initialize_mask(dst_idxes)),
...
#undef KDDIM
    ...
    //--------------------------------------------------------------------------
    {
      ...
      if (IS_REDUCE(usage))
      {
        ...
        for (unsigned idx = 0; idx < sources.size(); idx++)
        {
          ...
          if (restricted_finder == restricted_instances.end())
          {
            restricted_instances[set_expr].insert(view, view_mask);
            set_expr->add_expression_reference();
            view->add_nested_valid_ref(did, &mutator);
          }
          else 
          {
#ifdef DEBUG_LEGION
            assert(view_mask * restricted_finder->second.get_valid_mask());
#endif
            if (restricted_finder->second.insert(view, view_mask))
              view->add_nested_valid_ref(did, &mutator);
          }
        }
      }
        ...
        for (unsigned idx = 0; idx < sources.size(); idx++)
        {
          ...
          if (restricted)
          {
            ...
            else
            {
              ...
              assert(view_mask * restricted_finder->second.get_valid_mask());
#endif
              restricted_finder->second.insert(view, view_mask);
            }
          }
        }
      }
    ...
    //--------------------------------------------------------------------------
    {
      ...
      else if (IS_WRITE(analysis.usage) && IS_DISCARD(analysis.usage))
      {
        ...
        if (!!non_restricted)
        {
          ...
          record_instances(expr, expr_covers, non_restricted,
              analysis.target_instances, analysis.target_views, mutator);
        }
        // Issue copy-out copies for any restricted fields
        if (!restricted_instances.empty())
        {
      ...
      else
      {
        ...
        update_set_internal(input_aggregator, RtEvent::NO_RT_EVENT, 
                            ...
                            analysis.target_instances, analysis.target_views,
                            analysis.source_views, applied_events, 
                            analysis.record_valid);
        if (IS_WRITE(analysis.usage))
        {
          update_initialized_data(expr, expr_covers, user_mask);
    ...
    //--------------------------------------------------------------------------
    {
      if (expr_covers)
      {
        for (unsigned idx = 0; idx < target_views.size(); idx++)
        {
          ...
          {
            target->add_nested_valid_ref(did, &mutator);
            total_valid_instances.insert(target, valid_mask);
          }
          else
            finder.merge(valid_mask);
        }
      ...
      else
      {
        ...
        for (unsigned idx = 0; idx < target_views.size(); idx++)
        {
          ...
          if (!valid_mask)
            continue;
          InstanceView *target = target_views[idx];
          need_rebuild = record_partial_valid_instance(target, expr,
                                                       valid_mask, mutator);
        }
        if (need_rebuild)
        {
    ...
    //--------------------------------------------------------------------------
    {
      ...
      for (LegionList<FieldSet<IndexSpaceExpression*> >::aligned::const_iterator
            it = restricted_sets.begin(); it != restricted_sets.end(); it++)
      {
        ...
        if (!diff_expr->is_empty())
        {
          ...
          {
            const FieldMask valid_mask = 
              target_instances[idx].get_valid_fields() & it->set_mask;
            if (!valid_mask)
              continue;
            InstanceView *target = target_views[idx];
#ifdef DEBUG_LEGION
...
#endif
#endif
            record_partial_valid_instance(target, diff_expr, 
                                          it->set_mask, mutator);
#ifdef DEBUG_LEGION
            assert(!need_rebuild);
#endif
      ...
      else
      {
        ...
        if (!(filter_mask * total_valid_instances.get_valid_mask()))
        {
                ...
                total_valid_instances.end(); it++)
          {
            ...
            LegionMap<LogicalView*,FieldMaskSet<IndexSpaceExpression> >::
             aligned::iterator finder = partial_valid_instances.find(it->first);
            if (finder != partial_valid_instances.end())
            {
#ifdef DEBUG_LEGION
              assert(finder->second.get_valid_mask() * overlap);
#endif
              if (finder->second.insert(diff_expr, overlap))
                diff_expr->add_expression_reference();
              // Remove duplicate reference
              if (add_reference && it->first->remove_nested_valid_ref(did))
                assert(false); // should never hit this
            }
            else
            {
              partial_valid_instances[it->first].insert(diff_expr, overlap);
              diff_expr->add_expression_reference();
              if (add_reference)
                it->first->add_nested_valid_ref(did);
            }
            partial_valid_fields |= overlap;
          }
          // Do not remove references when deleting, they flowed back
          for (std::vector<LogicalView*>::const_iterator it =
                to_delete.begin(); it != to_delete.end(); it++)
            total_valid_instances.erase(*it);
    ...
    void EquivalenceSet::filter_unrestricted_instances(
      IndexSpaceExpression *expr, const bool expr_covers, FieldMask filter_mask)
    //--------------------------------------------------------------------------
    {
    ...
    //--------------------------------------------------------------------------
    {
      ...
      make_instances_valid(input_aggregator, guard_event, op, index, 
                       ...
                       target_instances, target_views, source_views, set_expr);
      const bool is_write = IS_WRITE(usage);
      const FieldMask reduce_mask = reduction_fields & user_mask;
      const FieldMask restricted_mask = restricted_fields & user_mask;
      if (!!reduce_mask)
      {
        ...
        FieldMaskSet<InstanceView> reduction_targets;
        for (unsigned idx = 0; idx < target_views.size(); idx++)
        {
          const FieldMask inst_mask = 
            target_instances[idx].get_valid_fields() & reduce_mask;
          if (!inst_mask)
            continue;
          InstanceView *target = target_views[idx];
#ifdef DEBUG_LEGION
          assert(inst_mask * reduction_targets.get_valid_mask());
#endif
          reduction_targets.insert(target, inst_mask);
        }
#ifdef DEBUG_LEGION
        assert(!reduction_targets.empty());
#endif
        FieldMaskSet<IndexSpaceExpression> applied_reductions;  
        apply_reductions(reduction_targets, expr, expr_covers, 
            reduction_targets.get_valid_mask(), input_aggregator, guard_event,
            op, index, false/*track*/, is_write ? NULL : &applied_reductions);
          ...
          if (!applied_reductions.empty())
          {
                  ...
                  it = reduced_sets.begin(); it != reduced_sets.end(); it++)
            {
              ...
              if (!!restricted_mask)
              {
                ...
                if (!!overlap)
                {
                  filter_unrestricted_instances(union_expr, 
                                    union_covers, overlap);
                  it->set_mask -= overlap;
                }
              }
              if (!!it->set_mask)
      ...
      if (record_valid)
      {
        ...
        if (!!restricted_mask)
        {
          ...
          if (!!non_restricted)
            record_instances(expr, expr_covers, non_restricted,
                             target_instances, target_views, mutator);
          record_unrestricted_instances(expr, expr_covers, restricted_mask,
                             target_instances, target_views, mutator);
        }
        else
    ...
    //--------------------------------------------------------------------------
    {
      ...
      {
        FieldMask inst_mask = 
          target_instances[idx].get_valid_fields() & update_mask;
        if (!inst_mask)
          continue;
        InstanceView *target = target_views[idx];
        if (!skip_check)
            ...
            update_instances.begin(); uit != update_instances.end(); uit++)
      {
            ...
            if (!!partial_valid)
            {
              ...
              if (!partial_exprs.empty())
              {
                      ...
                      partial_exprs.begin(); it != partial_exprs.end(); it++)
                {
                  ...
                  if (!diff_expr->is_empty())
                    issue_update_copies_and_fills(uit->first, source_views,
                       aggregator, guard_event, op, index, track_events, 
                       diff_expr, false/*expr covers*/, it->second, dst_index); 
                }
        ...
        if (!!uit->second)
          issue_update_copies_and_fills(uit->first, source_views, aggregator, 
                                  guard_event, op, index, track_events, expr, 
                                  expr_covers, uit->second, dst_index);
      }
    }
    ...
    //--------------------------------------------------------------------------
    {
              ...
              remainders.begin(); it != remainders.end(); it++)
        {
          if (it->first != expr)
          {
            ...
            issue_update_copies_and_fills(target, empty_sources, aggregator,
                ...
                false/*covers*/, it->second, dst_index);
          }
      ...
      if (!!total_fields)
      {
        if (total_fields != total_valid_instances.get_valid_mask())
        {
          ...
          aggregator->record_updates(target, total_instances,total_fields,expr);
        }
        ...
        while (fidx >= 0)
        {
          ...
          if (expr_covers)
          {
            if (aggregator == NULL)
              aggregator = new CopyFillAggregator(runtime->forest, op, index,
                                                  guard_event, track_events);
            aggregator->record_reductions(rit->first,finder->second,fidx,fidx);
    ...
    //--------------------------------------------------------------------------
    {
            ...
            rit != restricted_instances.end(); rit++)
      {
        ...
        LegionMap<std::pair<InstanceView*,InstanceView*>,FieldMask>::aligned
          restricted_copies;
        unique_join_on_field_mask_sets(rit->second, sources, restricted_copies);
        if (restricted_copies.empty())
          continue;
        if (aggregator == NULL)
          aggregator = new CopyFillAggregator(runtime->forest, op, index,
                                    RtEvent::NO_RT_EVENT, true/*track*/);
        for (LegionMap<std::pair<InstanceView*,InstanceView*>,FieldMask>::
