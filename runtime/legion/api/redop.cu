...
namespace Legion {
  namespace Internal {
    ...
    public:
      ...
      static __device__ void apply_cuda(
          typename T::LHS& lhs, typename T::RHS rhs)
      {
        T::template apply<EXCLUSIVE>(lhs, rhs);
      }

      template<bool EXCLUSIVE>
      static __device__ void fold_cuda(
          typename T::LHS& lhs, typename T::RHS rhs)
      {
    ...
    public:
      ...
      template<bool EXCLUSIVE>
      static __device__ void apply_hip(
          typename T::LHS& lhs, typename T::RHS rhs)
      {
        T::template apply<EXCLUSIVE>(lhs, rhs);
      }
      ...
      template<bool EXCLUSIVE>
      static __device__ void fold_hip(typename T::LHS& lhs, typename T::RHS rhs)
      {
        T::template fold<EXCLUSIVE>(lhs, rhs);
      }
    };
...
#define REGISTER_BUILTIN_REDOP_CUDA(id, type)         \
  runtime_register_reduction_op(                      \
      id,                                             \
      Realm::ReductionOpUntyped::create_reduction_op< \
          AddCudaReductions<type> >(),                \
