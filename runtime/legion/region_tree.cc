...
namespace Legion {
  namespace Internal {
    ...
    //--------------------------------------------------------------------------
    ApEvent RegionTreeForest::physical_register_only(
                                           const RegionRequirement &req,
                                           VersionInfo &version_info,
    ...
    //--------------------------------------------------------------------------
    {
      ...
      // This is a very strange case that occurs for control replication 
      // where we need to sync multiple operations to know they've done all
      // their copies before we register the users. Don't think too hard 
      // about this unless you are thinking about inline mapping operations
      // for control replication.
      if (sync_add_users != NULL)
      {
    ...
    //--------------------------------------------------------------------------
    void RegionTreeForest::acquire_restrictions(
                                           const RegionRequirement &req,
                                           VersionInfo &version_info,
                                           Operation *op, unsigned index,
                                           ApEvent term_event,
    ...
    //--------------------------------------------------------------------------
    void RegionTreeForest::release_restrictions(
                                           const RegionRequirement &req,
                                           VersionInfo &version_info,
                                           Operation *op, unsigned index,
                                           ApEvent precondition,
