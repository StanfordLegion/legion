...
namespace Legion {
    ...
    template<>
    IndexPartition Runtime::create_index_partition(Context ctx,
        IndexSpace parent, const Blockify<1>& mapping, int part_color)
    //--------------------------------------------------------------------------
    {
      LegionRuntime::Arrays::Rect<1> parent_rect =
        get_index_space_domain(ctx, parent).get_rect<1>();
      LegionRuntime::Arrays::Point<1> origin = parent_rect.lo;
      parent_rect -= origin;

      LegionRuntime::Arrays::Rect<1> color_space =
        mapping.image_convex(parent_rect);
      DomainPointColoring c;
      for (typename Blockify<1>::PointInOutputRectIterator pir(color_space);
          pir; pir++)
      {
        LegionRuntime::Arrays::Rect<1> preimage = mapping.preimage(pir.p);
        c[DomainPoint::from_point<1>(pir.p)] =
          Domain::from_rect<1>(preimage.intersection(parent_rect) + origin);
      }
      IndexPartition result = create_index_partition(ctx, parent,
              Domain::from_rect<1>(color_space), c,
    ...
    template<>
    IndexPartition Runtime::create_index_partition(Context ctx,
        IndexSpace parent, const Blockify<2>& mapping, int part_color)
    //--------------------------------------------------------------------------
    {
      LegionRuntime::Arrays::Rect<2> parent_rect =
        get_index_space_domain(ctx, parent).get_rect<2>();
      LegionRuntime::Arrays::Point<2> origin = parent_rect.lo;
      parent_rect -= origin;

      LegionRuntime::Arrays::Rect<2> color_space =
        mapping.image_convex(parent_rect);
      DomainPointColoring c;
      for (typename Blockify<2>::PointInOutputRectIterator pir(color_space);
          pir; pir++)
      {
        LegionRuntime::Arrays::Rect<2> preimage = mapping.preimage(pir.p);
        c[DomainPoint::from_point<2>(pir.p)] =
          Domain::from_rect<2>(preimage.intersection(parent_rect) + origin);
      }
      IndexPartition result = create_index_partition(ctx, parent,
              Domain::from_rect<2>(color_space), c,
              DISJOINT_KIND, part_color);
      return result;
    }
    ...
    template<>
    IndexPartition Runtime::create_index_partition(Context ctx,
        IndexSpace parent, const Blockify<3>& mapping, int part_color)
    //--------------------------------------------------------------------------
    {
      LegionRuntime::Arrays::Rect<3> parent_rect =
        get_index_space_domain(ctx, parent).get_rect<3>();
      LegionRuntime::Arrays::Point<3> origin = parent_rect.lo;
      parent_rect -= origin;

      LegionRuntime::Arrays::Rect<3> color_space =
        mapping.image_convex(parent_rect);
      DomainPointColoring c;
      for (typename Blockify<3>::PointInOutputRectIterator pir(color_space);
          pir; pir++)
      {
        LegionRuntime::Arrays::Rect<3> preimage = mapping.preimage(pir.p);
        c[DomainPoint::from_point<3>(pir.p)] =
          Domain::from_rect<3>(preimage.intersection(parent_rect) + origin);
      }
      IndexPartition result = create_index_partition(ctx, parent,
              Domain::from_rect<3>(color_space), c,
              DISJOINT_KIND, part_color);
      return result;
    }
