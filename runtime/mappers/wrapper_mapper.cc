...
namespace Legion {
  namespace Mapping{
    ...
    std::string WrapperMapper::Serialize(const std::vector<std::string> &tasks_print, const std::vector<std::string> &tasks_stop,  const std::map<int, int> &procs_map){
      ...
      for (std::vector<std::string>::const_iterator i = tasks_print.begin(); i != tasks_print.end(); ++i){
        send_string = send_string + *i +  "\\";
      }
      //std::cout<<send_string;
      send_string = send_string + "#";
      for (std::vector<std::string>::const_iterator i = tasks_stop.begin(); i != tasks_stop.end(); ++i){
        send_string = send_string + *i + "\\";
      }
    ...
    void WrapperMapper::get_input(const MapperContext(ctx)){
      ...
      std::cout<< "Enter \"help\" to see the list of commands. Enter \"exit\" to exit.\n";
      std::cout<<">    ";
      while (1)
      {
...
void WrapperMapper::get_select_task_options_input(const MapperContext ctx, std::string task_name, TaskOptions& output){
  std::string strValue;
  std::cout<<"\nEnter \"change\" to change the list of tasks and processors being monitored. Enter \"help\" for the list of commands. Enter \"exit\" to exit\n";
...
void WrapperMapper::get_map_task_input(Task *task){
  std::string strValue;
  std::cout<< "Enter \"change\" to change the list of tasks and methods being monitored. Enter \"exit\" to exit.\n>    ";			
  while (1)
  {
    getline(std::cin, strValue); 
...
void WrapperMapper::select_task_options(const MapperContext    ctx,
  ...
  std::vector<std::string>::iterator ittp = std::find(print_tasks.begin(), print_tasks.end(), task.get_task_name());  
  std::vector<std::string>::iterator itts = std::find(stop_tasks.begin(), stop_tasks.end(), task.get_task_name());  
  ...
  void WrapperMapper::handle_message(const MapperContext           ctx,
    ...
    if (node_id==0 && ownerprocessor.id==local_proc.id){
      if (rec_message->tag==42356156){
        ...
        if (action && (unsigned)task_int<print_tasks.size()) task_name = print_tasks[task_int]; 
        else if (!action && (unsigned)task_int<stop_tasks.size()) task_name = stop_tasks[task_int];
        else action=2;
        if (action==1 || action==0){
          std::cout<<"\n--------------HTASK: "<<task_name<<" FUNCTION: select_task_options--------------\n";
        ...
        //select_task_options_message mess ={42356156,"abc",output, action};
