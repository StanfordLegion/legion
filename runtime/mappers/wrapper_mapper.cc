...
*/
		...
		void WrapperMapper::get_select_task_options_input(const MapperContext ctx, const Task& task, TaskOptions& output){
			std::string strValue;
...
std::map<std::string, int>::iterator itt = WrapperMapper::tasks_map.find(task.get_task_name());
		...
		if(WrapperMapper::ownerprocessor==local_proc){
				...
				if (((itt!=tasks_map.end() && itt->second==0) && (itm!=methods_map.end() && itm->second==0))||(itp!=procs_map.end() && itp->second==0)) {
					...
					WrapperMapper::get_select_task_options_input(ctx, task, output);
				}
				}
			else{

				select_task_options_message message ={task,output,"select_task_options_message"};
				void *message_point = &message;
...
/*else if ((get_input_message)*message.message->tag="get_input_message"){
}*/
else mapper_rt_trigger_mapper_event(ctx, WrapperMapper::mapevent);
}
