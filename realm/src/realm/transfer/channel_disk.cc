...
namespace Realm {
  ...
  FileXferDes::FileXferDes(uintptr_t _dma_op, Channel *_channel, NodeID _launch_node,
                           XferDesID _guid,
  ...
  bool FileXferDes::progress_xd(FileChannel *channel, TimeLimit work_until)
  {
    Request *rq;
    bool did_work = false;
    do {
  ...
  void FileXferDes::flush() { fsync(file_info->fd); }

  DiskXferDes::DiskXferDes(uintptr_t _dma_op, Channel *_channel, NodeID _launch_node,
                           XferDesID _guid,
                           const std::vector<XferDesPortInfo> &inputs_info,
                           const std::vector<XferDesPortInfo> &outputs_info,
                           int _priority)
  ...
  bool DiskXferDes::progress_xd(DiskChannel *channel, TimeLimit work_until)
  {
    Request *rq;
    bool did_work = false;
    do {
  ...
  void DiskXferDes::flush() { fsync(fd); }
  ...
  XferDes *FileChannel::create_xfer_des(uintptr_t dma_op, NodeID launch_node,
                                        XferDesID guid,
                                        const std::vector<XferDesPortInfo> &inputs_info,
                                        const std::vector<XferDesPortInfo> &outputs_info,
                                        ...
                                        size_t fill_total)
  {
    ...
    assert(fill_size == 0);
    return new FileXferDes(dma_op, this, launch_node, guid, inputs_info, outputs_info,
                           priority);
  }
  ...
  long FileChannel::submit(Request **requests, long nr)
  {
    ...
    for(long i = 0; i < nr; i++) {
      ...
      switch(req->xd->kind) {
      case XFER_FILE_READ:
        aio_ctx->enqueue_read(req->fd, req->file_off, req->nbytes, req->mem_base, req);
        break;
      case XFER_FILE_WRITE:
        aio_ctx->enqueue_write(req->fd, req->file_off, req->nbytes, req->mem_base, req);
        break;
      default:
        assert(0);
      }
  ...
  XferDes *DiskChannel::create_xfer_des(uintptr_t dma_op, NodeID launch_node,
                                        XferDesID guid,
                                        const std::vector<XferDesPortInfo> &inputs_info,
                                        const std::vector<XferDesPortInfo> &outputs_info,
                                        int priority, XferDesRedopInfo redop_info,
                                        ...
                                        size_t fill_total)
  {
    assert(redop_info.id == 0);
    assert(fill_size == 0);
    return new DiskXferDes(dma_op, this, launch_node, guid, inputs_info, outputs_info,
                           priority);
  }
  ...
  long DiskChannel::submit(Request **requests, long nr)
  {
    ...
    for(long i = 0; i < nr; i++) {
      ...
      switch(req->xd->kind) {
      case XFER_DISK_READ:
        aio_ctx->enqueue_read(req->fd, req->disk_off, req->nbytes, req->mem_base, req);
        break;
      case XFER_DISK_WRITE:
        aio_ctx->enqueue_write(req->fd, req->disk_off, req->nbytes, req->mem_base, req);
        break;
      default:
        assert(0);
      }
