                    ...
                    Context ctx, HighLevelRuntime *runtime)
{
  ...
  // the cause of it.
  {
    ...
    for (int color = 0; color < num_subregions; color++)
    {
      ...
      if (color == 0)
        spmd_launcher.add_region_requirement(
            RegionRequirement(ghost_left[num_subregions-1], READ_WRITE,
                              SIMULTANEOUS, ghost_left[num_subregions-1]));
      else
        spmd_launcher.add_region_requirement(
            RegionRequirement(ghost_left[color-1], READ_WRITE,
                              SIMULTANEOUS, ghost_left[color-1]));
      // Right Ghost
      if (color == (num_subregions-1))
        spmd_launcher.add_region_requirement(
            RegionRequirement(ghost_right[0], READ_WRITE,
                              SIMULTANEOUS, ghost_right[0]));
      else
        spmd_launcher.add_region_requirement(
            RegionRequirement(ghost_right[color+1], READ_WRITE,
  ...
  for (int s = 0; s < args->num_steps; s++)
  {
    ...
    for (unsigned idx = GHOST_LEFT; idx <= GHOST_RIGHT; idx++)
    {
      ...
      if (s > 0)
      {
        ...
        // Once we have registered the dependence on the phase
        // barrier we can advance the barrier to the next phase
        args->wait_empty[idx] = 
