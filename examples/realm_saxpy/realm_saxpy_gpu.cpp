                    ...
                    const void *userdata, size_t userlen, Processor p)
{
  ...
  if(TestConfig::prefetch) {
    ...
    if(saxpy_args->x_inst.get_location().kind() == Memory::Kind::GPU_MANAGED_MEM) {
      hipMemAdvise(&ra_x[saxpy_args->bounds.lo], num_elements * sizeof(float),
                    hipMemAdviseSetReadMostly, 0 /*unused*/);
      hipMemPrefetchAsync(&ra_x[saxpy_args->bounds.lo],
                           num_elements * sizeof(float), device,
                           0 /*default stream*/);
    }

    if(saxpy_args->y_inst.get_location().kind() == Memory::Kind::GPU_MANAGED_MEM) {
      hipMemAdvise(&ra_y[saxpy_args->bounds.lo], num_elements * sizeof(float),
                    hipMemAdviseSetReadMostly, 0 /*unused*/);
      hipMemPrefetchAsync(&ra_y[saxpy_args->bounds.lo],
                           num_elements * sizeof(float), device,
                           0 /*default stream*/);
    }
    ...
    if(saxpy_args->z_inst.get_location().kind() == Memory::Kind::GPU_MANAGED_MEM) {
      ...
      hipMemPrefetchAsync(&ra_z[saxpy_args->bounds.lo],
                           num_elements * sizeof(float), device,
                           0 /*default stream*/);
    }
  }
  ...
  hipLaunchKernelGGL(gpu_saxpy, total_ctas, cta_threads, 0, hipGetTaskStream(), saxpy_args->alpha, saxpy_args->bounds,
					 ra_x, ra_y, ra_z);
  // LOOK: NO WAIT! :)
}
