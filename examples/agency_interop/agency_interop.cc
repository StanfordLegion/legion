                     ...
                     Context ctx, Runtime *runtime)
{
  ...
  Rect<1> subrect; ByteOffset stride;
  double *ptr = acc.raw_dense_ptr<1>(rect, subrect, stride);
                ...
                Context ctx, Runtime *runtime)
{
  ...
  Rect<1> rect = dom.get_rect<1>();
  Rect<1> subrect; ByteOffset stride;
  double *x_ptr = acc_x.raw_dense_ptr<1>(rect, subrect, stride);
  assert((x_ptr != NULL) && (rect == subrect) && (stride.offset == sizeof(double)));
  double *y_ptr = acc_y.raw_dense_ptr<1>(rect, subrect, stride);
  assert((y_ptr != NULL) && (rect == subrect) && (stride.offset == sizeof(double)));
  double *z_ptr = acc_z.raw_dense_ptr<1>(rect, subrect, stride);
  assert((z_ptr != NULL) && (rect == subrect) && (stride.offset == sizeof(double)));
                ...
                Context ctx, Runtime *runtime)
{
  ...
  {
    double x = acc_x.read(DomainPoint::from_point<1>(pir.p));
