  ...
  namespace MPI {
    ...
    int AM_Init(int *p_node_this, int *p_node_size)
    {
      ...
      if(mpi_thread_model < MPI_THREAD_MULTIPLE) {
        fprintf(stderr, "MPI: Unsupported threading module found for MPI, please ensure "
      ...
      *p_node_this = node_this;
      ...
      for(int i = 0; i < n_am_mult_recv; i++) {
        CHECK_MPI(MPI_Irecv(buf_recv_list[i], AM_BUF_SIZE_W_HEADER, MPI_CHAR,
                            MPI_ANY_SOURCE, TAG_COMMAND, MPI_COMM_WORLD,
                            &req_recv_list[i]));
      }
    ...
    static void incoming_message_handled(NodeID sender, uintptr_t comp_ptr,
                                         uintptr_t /*unused*/)
    {
      struct AM_msg msg;
      ...
      CHECK_MPI(MPI_Send(&msg, AM_MSG_HEADER_SIZE, MPI_CHAR, sender, TAG_COMMAND,
                         MPI_COMM_WORLD));
    ...
    void AMPoll()
    {
      ...
      while(1) {
        ...
        if(msg->type == 0) {
          ...
          CHECK_MPI(MPI_Recv(payload, msg->payload_size, MPI_BYTE, tn_src, msg_tag,
                             comm_medium, &status));
        } else if(msg->type == 2) {
          int32_t offset;
        ...
        if(msg->msgid != 0x7fff) {
          bool handled = g_message_manager->add_incoming_message(
              tn_src, msg->msgid, header, msg->header_size, PAYLOAD_COPY, payload,
        ...
        CHECK_MPI(MPI_Irecv(buf_recv_list[i_recv_list], AM_BUF_SIZE_W_HEADER, MPI_CHAR,
                            MPI_ANY_SOURCE, TAG_COMMAND, MPI_COMM_WORLD,
                            &req_recv_list[i_recv_list]));
                ...
                int has_dest, MPI_Aint dest, void *remote_comp)
    {
      ...
      if(has_dest) {
        ...
        if(payload_lines > 1) {
          ...
          for(int i = 0; i < payload_lines; i++)
            CHECK_MPI(MPI_Put(payload + (i * payload_line_stride), line_size, MPI_BYTE,
                              tgt, dest + (i * line_size), line_size, MPI_BYTE,
                              g_am_win));
        } else
          CHECK_MPI(MPI_Put(payload, payload_size, MPI_BYTE, tgt, dest, payload_size,
                            MPI_BYTE, g_am_win));
        ...
        assert(payload_lines <=
               1); // TODO: multi-line means sending line count/stride to receiver
        CHECK_MPI(
            MPI_Send(payload, payload_size, MPI_BYTE, tgt, msg_tag, MPI_COMM_WORLD));
#endif
