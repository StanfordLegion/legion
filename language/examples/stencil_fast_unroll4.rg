...
task main()
  ...
  do
    ...
    for t = 0, tsteps, 4 do
      ...
      -- __demand(__parallel)
      for i = 0, nt2 do
        stencil(private[i], interior[i], pxm_in[i], pxp_in[i], pym_in[i], pyp_in[i], t + 1 == tprune)
      end
      -- __demand(__parallel)
      for i = 0, nt2 do
        increment(private[i], exterior[i], pxm_out[i], pxp_out[i], pym_out[i], pyp_out[i], t + 1 == tsteps - tprune - 1)
      end
      -- __demand(__parallel)
      for i = 0, nt2 do
        stencil(private[i], interior[i], pxm_in[i], pxp_in[i], pym_in[i], pyp_in[i], t + 2 == tprune)
      end
      -- __demand(__parallel)
      for i = 0, nt2 do
        increment(private[i], exterior[i], pxm_out[i], pxp_out[i], pym_out[i], pyp_out[i], t + 2 == tsteps - tprune - 1)
      end
      -- __demand(__parallel)
      for i = 0, nt2 do
        stencil(private[i], interior[i], pxm_in[i], pxp_in[i], pym_in[i], pyp_in[i], t + 3 == tprune)
      end
      -- __demand(__parallel)
      for i = 0, nt2 do
        increment(private[i], exterior[i], pxm_out[i], pxp_out[i], pym_out[i], pyp_out[i], t + 3 == tsteps - tprune - 1)
      end
    end
