...
__demand(__parallel)
task init(r1 : region(ispace(int2d), fs1), r2 : region(fs2), size : int)
...
do
  ...
  for e in r2 do
    e.g = 0
    e.h = 0
    e.p = int2d { __raw(e).value / size, __raw(e).value % size }
  end
end

__demand(__parallel)
task stencil(r1 : region(ispace(int2d), fs1), r2 : region(fs2))
...
task stencil_serial(r1 : region(ispace(int2d), fs1), r2 : region(fs2))
...
task check(r1 : region(ispace(int2d), fs1), r2 : region(fs2))
where reads(r2.{g, h})
do
  for e in r2 do
