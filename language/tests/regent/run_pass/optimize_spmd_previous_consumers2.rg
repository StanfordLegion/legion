...
--   * multiple previous consumers of a region
...
where reads(q.a), reads writes(r.c) do
  var s, t = 0, 0
  for y in q do
    s += y.a
    t += 1
...
task avg_ad(r : region(t), q : region(t))
where reads(q.a), reads writes(r.d) do
  var s, t = 0, 0
  for y in q do
    s += y.a
    t += 1
...
task rot_cdb(r : region(t), y : int, z : int)
where reads(r.c, r.d), reads writes(r.b) do
  for x in r do
    if x.c >= 0 then
      x.b = (x.c + y)*x.d % z
    else
...
task main()
  ...
  regentlib.assert(x0.a ==  9958, "test failed")
  regentlib.assert(x1.a == 10227, "test failed")
  regentlib.assert(x2.a == 10357, "test failed")
  regentlib.assert(x3.a == 10163, "test failed")
end
