...
function make_tasks(ty)
  ...
  do
    for e in r do
      ...
      r[e].output_gpu.v_add1 = [ty](0)
      r[e].output_gpu.v_add2 = [ty](0)
  ...
  do
    ...
    __parallel_prefix(r.output_gpu.v_add2, r.output_gpu.v_add1,   +, dir2)
  end
  ...
  do
    ...
    r[bounds.hi].temp.v_add2 = r[bounds.hi].temp.v_add1
    for i = [int64](bounds.hi) - 1, [int64](bounds.lo) - 1, -1 do
      r[i].temp.v_add2 = r[i + 1].temp.v_add2 + r[i].temp.v_add1
    end
    ...
    for e in r do
      [(function()
        if DEBUG then
          return rquote
            regentlib.c.printf([format_strings[ty] .. ", " .. format_strings[ty] .. "\n"], e.temp.v_add1, e.output_gpu.v_add1)
            regentlib.c.printf([format_strings[ty] .. ", " .. format_strings[ty] .. "\n"], e.temp.v_add2, e.output_gpu.v_add2)
          end
        else
          return rquote end
        end
