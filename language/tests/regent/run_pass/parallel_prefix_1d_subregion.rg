-- Copyright 2026 Stanford University, Los Alamos National Laboratory
--
...
local partition_symbols = {}
for i = 1, #types do
  region_symbols[types[i]] = regentlib.newsymbol("r_" .. tostring(types[i]))
  partition_symbols[types[i]] = regentlib.newsymbol("p_" .. tostring(types[i]))
end

task test(size : int64, n : int64)
  [types:map(function(ty)
    local fs = fs_types[ty]
    local region_sym = region_symbols[ty]
    local partition_sym = partition_symbols[ty]
    local init = init_tasks[ty]
    local prefix = prefix_tasks[ty]
    local check = check_tasks[ty]
    return rquote
      var [region_sym] = region(ispace(int1d, size), fs)
      var [partition_sym] = partition(equal, [region_sym], ispace(int1d, n))
      init([region_sym])
      for c in [partition_sym].colors do
        prefix([partition_sym][c])
      end
      for c in [partition_sym].colors do
        check([partition_sym][c])
      end
    end
  end)]
end
