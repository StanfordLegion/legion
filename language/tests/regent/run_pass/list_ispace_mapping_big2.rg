...
-- function returns large, unsigned 64-bit values.

import "regent"

local c = regentlib.c

terra mapping(p : int2d, r : rect2d) : uint64
  var size = r:size()
  var offset : uint64 = p.__ptr.y + p.__ptr.x * size.__ptr.y
  if offset % 2 == 0 then
    return 13835058055282163712ULL - (offset << 35)
  else
    return 13835058055282163712ULL + (offset << 35)
  end
end
