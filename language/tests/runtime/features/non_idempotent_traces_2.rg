...
task read_writer(r : region(ispace(int1d), int))
where
  reads(r), writes(r)
do
  for i in r do
...
do
  for x in r do
    var str : int8[256]
    regentlib.c.sprintf([&int8](str), "%d %d", r[x], 4 * (iter + 1))
    regentlib.assert(r[x] == (4 * (iter + 1)), str)
  end
end
...
task main()
  ...
  for j = 0, num_loops do
    -- We'll force different preconditions to happen by making
    -- the region be either be reduced to or written to.
    if j % 2 == 0 then
      for i in is1 do
        read_writer(p1[i])
      end
    ...
    -- As with non_idempotent_trace_1, we'll produce a non-idempotent
    -- trace by making the trace end with a reduction.
